...
public class TroopManagement {
		...
		switch(u.unitType()) {
			...
			case Worker:
				unit = new Worker(u.id(), gc, earthBattleMap, u.location().mapLocation(), 
						mineLocs, minedOut, Worker.Occupation.Miner, unitHashMap);
	...
	public void addBuilder(Unit u) {
		AbstractUnit unit = new Worker(u.id(), gc, earthBattleMap, u.location().mapLocation(),
				mineLocs, minedOut, Worker.Occupation.Builder, unitHashMap);
	...
	public void build(Worker worker) {
		...
		switch(worker.state) {
		   ...
		   // if case is Mine, do whatever Idle tells you to do
			...
			case Idle:
				Unit factory = worker.getNearbyUnbuiltStruct();
	...
	public void removeDeadWorkersAndStructures() {
		...
		for (int i : indices) {
			...
			removeUnit(unit.id);
			System.out.println(workersAndStructs.remove(i));
			wasaIndex--;
		}
