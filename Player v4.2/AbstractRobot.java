...
public abstract class AbstractRobot extends AbstractUnit {
	public enum State { Build, Move, Mine, Idle, Garrison, Spawn; };
	public int range[] = {72, 50, 32, 18, 8, 2};
	...
	public int movePath(boolean moveToEnemy) {
		...
		if (!gc.canMove(id, dir)) {
			...
			// if the obstruction is a worker or a factory or rocket
			if (unit.unitType() == UnitType.Worker || unit.unitType() == 
					UnitType.Factory || unit.unitType() == UnitType.Rocket) {
		...
		gc.moveRobot(id, dir);
		attempts = 0;
		// set previous location's occupant to -1
	...
	public int attack() {
		...
		// iterate through the units and try to attack factory or rockets
		for (int i = 0; i < vecUnit.size(); i++) {
			Unit enemyUnit = vecUnit.get(i);
			
			if (enemyUnit.unitType() != UnitType.Factory || 
					enemyUnit.unitType() != UnitType.Rocket) continue;
			
			if (gc.canAttack(id, enemyUnit.id())) {
				gc.attack(id, enemyUnit.id());
				System.out.println("SUCCESSFULLY ATTACKED FACTORY OR ROCKET");
				return 1;
			}
		}
	...
	public int moveTowardsEnemy() {
		...
		if (dir == null) {
			...
			Direction nextDir = Utility.nextDirection(dir);
			...
			// try an adjacent direction
			Direction previousDir = Utility.previousDirection(dir);
			...
			// try an adjacent direction
			setDirection(Utility.nextDirection(nextDir));
			returnValue = moveDir();
			
			if (returnValue != 2) return returnValue;
			
			// try an adjacent direction
			setDirection(Utility.previousDirection(previousDir));
			returnValue = moveDir();
	...
	/**
	 * Looks for enemies and moves away from them if health is low.
	 * @return
	 * 	1 if moved away form enemy
	 * 	0 if health is sufficient 
	 * 	-1 if was unable to
	 */
	public int moveAwayFromEnemy() {
		int returnValue;
		
		Direction enemyDir = enemyDirection();
		
		if (enemyDir == null) return -1;
		Direction dir = Utility.getOppositeDirection(enemyDir);
	...
	/**
	 * Tries to load a robot into a factory or a rocket
	 * @return
	 * 	Returns 1 if successfully loaded
	 * 	Returns 2 if failed to load
	 */
	public int load(int structureId) {
		if (!gc.canLoad(structureId, id)) {
			return 2;
		}
		
		gc.load(structureId, id);
		resetPath();
	 ...
	 */
	public MapLocation closestEnemyLocation() {
		...
		return vecUnit1.get(0).location().mapLocation();
	}
	...
	public MapLocation getClosestLocation(MapLocation location, 
			ArrayList<MapLocation> destinations) {
		MapLocation dest;
		int xDiff, yDiff;
		int distance;
		...
		for (int i = 0; i < destinations.size(); i++) {
			...
			xDiff = Math.abs(location.getX() - dest.getX());
	...
	public Tuple getClosestLocation(ArrayList<Tuple> destinations, 
		...
		for (int i = 0; i < destinations.size(); i++) {
			dest = destinations.get(i);
			xDiff = Math.abs(location.getX() - dest.x);

			// the difference in y values of the two points
		   yDiff = Math.abs(location.getY() - dest.y);
