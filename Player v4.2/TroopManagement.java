...
public class TroopManagement {
	...
	TroopManagement(GameController g, UnitCounter uc) {
		...
		wasaIndex = 0;
		System.out.println("Total Karbonite " + ((BattleMap) battleMap).totalKarbonite);
	}
	...
	public void mine(Worker worker) {
		...
		if (worker.movePath != null) {
			if (worker.movePath(false) == 4) {
				battleMap.unreserveLocation(new MapLocation(planet, 
					worker.mineDestination.x, worker.mineDestination.y));
			}
			return;
		}
		...
		if (gc.karboniteAt(worker.currentLocation) > 0) {
			worker.mine();
			return;
		}	
		...
		path = worker.getMinePath();
		
		// there is a path
		if (path != null) {
			worker.setPath(path);
			worker.mineDestination = new Tuple(path[path.length-1].getX(), path[path.length-1].getY());
	...
	public void work(Worker worker, UnitType structureType) {
		...
		Unit structure = worker.getNearbyUnbuiltStruct(20);
		...
		if (structure != null) {
			if (worker.setPath(worker.getLocation(), structure.location().
				...
				battleMap.unreserveLocation(worker.currentLocation);
	...
	/*	if (factory.factoryCount % 3 == 0) {
			produce(factory, UnitType.Knight);
		} else if (factory.factoryCount % 3 == 1) {
		...
		if (factory.factoryCount % 2 == 0) {
			produce(factory, UnitType.Ranger);
		} else if (factory.factoryCount % 2 == 1) {
			produce(factory, UnitType.Healer);
		}
	}
	...
	public void goToEnemy(AbstractRobot robot) {
		...
		// next turn
		int returnValue = robot.attack();
		if (returnValue == 1 || returnValue == 2) {
			robot.moveAwayFromEnemy();
			return;
		}
	...
	public void goToEnemyKnight(AbstractRobot robot) {
		if (robot.attack() == 1) {
			return;
		}
		
		MapLocation dest = robot.closestEnemyLocation();
		...
		} else if (robot.setPath(robot.currentLocation, dest)) {
			if (robot.movePath(false) == 3) {
				robot.moveTowardsEnemy();
			}
		} else {
			robot.randomMovement();
		}
	...
	public void goToEnemyHealer(Healer healer) {
		// if the robot attacked, it shouldn't move because it can keep attacking 
		// next turn
		int returnValue = healer.attack();
		if (returnValue == 1 || returnValue == 2) {
			healer.moveAwayFromEnemy();
			return;
		}
		
		// first try moving toward the ally
		if (healer.moveTowardsEnemy() == 1) {
		// do nothing and attack at the end
			
		// if you can't move towards the enemy then try moving along the path
		// toward the enemy starting location
		} else if (healer.moveAwayFromEnemy() == 1) {
			
		} else if (healer.movePath != null) {
			healer.movePath(true);
		
		// robot.movePath == null, try setting a path, otherwise move randomly
		} else {
			if (healer.setPathToEnemy()) {
				healer.movePath(true);
				return;
			}
			
			healer.randomMovement();
		}
	...
	public void initializeWorkers(GameController gc) {
   	...
   	unitCounter.workerCap = 5 + (initialKarbonite / 500);
	if (unitCounter.workerCap > 50) unitCounter.workerCap = 50;
...
/*		long round = gc.round();
		if (round > 15 && unitCounter.factoryCount == 0) {
			return false;		
		}*/
	...
	/**
	 * Strategy for troop rush on mars
	 * @param robot
	 */
	public void goToEnemyMars(AbstractRobot robot) {
		// if the robot attacked, it shouldn't move because it can keep attacking 
		// next turn
