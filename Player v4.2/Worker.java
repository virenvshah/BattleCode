...
public class Worker extends AbstractRobot {
	...
	public Unit layBlueprint(Direction dir, UnitType structureType) {
		...
		battleMap.reserveLocation(blueprintLocation);
	...
	public int mine() {
		...
		if (gc.canHarvest(id, Direction.Center)) {
			...
			battleMap.decrementKarbonite(currentLocation);
			battleMap.reserveLocation(currentLocation);
			state = State.Mine;
			
			if (gc.karboniteAt(currentLocation) == 0) {
				System.out.println(battleMap.getTileType(currentLocation));
	...
	/**
	 * Returns the path to the closest mine location
	 */
	public MapLocation[] getMinePath() {
		return battleMap.minePath(currentLocation);
	}
