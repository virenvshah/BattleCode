...
public class Rocket extends AbstractStructure {
	 ...
	 */
	 ...
	 /*
	  * Launches the rocket to a given location
		* @param landingZone
	  * Where the rocket will end up
		* @return
		* -1 if we can't
		*  0 if we just did (and we'll be in spaaaaaaaace)
		* NOTE this doesn't take into account neighboring units: a precondition
		* is that we build the rocket in the right spot
	  */
		public int launch(MapLocation landingZone){
			if gc.canLaunchRocket(id,landingZone){
				gc.launchRocket(id,landingZone);
				return 0;
			}
			return -1;
		}
