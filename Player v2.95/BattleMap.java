...
public class BattleMap implements Map {
	...
	GameController gc;
	public int totalKarbonite; // the total Karbonite on the map
	...
	public BattleMap(PlanetMap pMap, Team ally, Team enemy, 
      ...
      makeGrid(planetMap);
      System.out.println("TotalKarbonite is " + totalKarbonite);
	}
	...
	public MapLocation[] minePath(MapLocation start) {
		...
		// the start and end points are the same
		if (tileNodeMap[startingPoint.y][startingPoint.x].tileType > 0) {
			return null;
		}
		...
		// set the weight for the starting node
		tileNodeMap[startingPoint.y][startingPoint.x].weight = 0;
		...
		while (frontier.size() > 0) {
			...
			for (int i = 0; i < tile.numOfNeighbors; i++) {
				...
				// if the goal has been reached
				if (successor.tileType > 0) {
					UnitType occ = getType(successor.occupantId);
					
					// if it is not occupied or reserved by another worker
					if (occ != UnitType.Factory && occ != UnitType.Rocket 
							&& occ != UnitType.Worker && !successor.reserved) {
						successor.predecessor = tile;
						successor.distance = tile.distance + 1;
						successor.reserved = true;
						return getPath(successor);
					}
				}
	...
	public void unreserveLocation(MapLocation loc) {
		tileNodeMap[loc.getY()][loc.getX()].reserved = false;
	}
	
	public void reserveLocation(MapLocation loc) {
		tileNodeMap[loc.getY()][loc.getX()].reserved = true;
	}
	...
	public void decrementKarbonite(MapLocation loc) {
		tileNodeMap[loc.getY()][loc.getX()].tileType -= 3;
		totalKarbonite -= 3;
		
		if (tileNodeMap[loc.getY()][loc.getX()].tileType < 0) {
			tileNodeMap[loc.getY()][loc.getX()].tileType = 0;
		}
	}
