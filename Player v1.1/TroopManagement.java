...
public class TroopManagement {
	...
	public void initializeWorkers(GameController gc, TroopManagement tm) {
   	...
   	for (int i = 0; i < vecUnit.size(); i++) {
      	...
      	tm.addUnit(u);
      	Worker worker = (Worker) tm.getUnit(u.id());
      	earthBattleMap.updateOccupant(worker.getLocation(), worker.id);
      	
      	ArrayList<Direction> dirs = earthBattleMap.
					getPassableDirections(worker.getLocation());
      	for (int j = 0; j < dirs.size(); j++) {
      		System.out.println(dirs.get(j));
      	}
			Direction dir = getRandomDirection(dirs);
      	System.out.println("Replicate " + worker.replicate(dir));
      	System.out.println("Worker location " + worker.getLocation().getX() + ", " + 
      			worker.getLocation().getY() + " Replication direction " + dir);
      	Unit replicatedWorker = gc.senseUnitAtLocation(
      			worker.getLocation().add(dir));
      	tm.addWorker(replicatedWorker, Worker.Occupation.Miner);
